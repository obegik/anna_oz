...
def median_perpos (sliced_data, output):
    ...
    sliced_data['read_name'] = 1
    median_perpos = sliced_data.groupby(['contig', 'position','reference_kmer', 'read_name']).agg({'median', 'count'})
...
def mean_perpos_perread (raw_data, output):
    ...
    mean_perpos_perread = raw_data.groupby(['contig', 'position','reference_kmer', 'read_name']).agg({'event_level_mean':['mean', 'count']})
    mean_perpos_perread.columns = ['mean', 'coverage']
...
def median_perpos_perread (raw_data, output):
    ...
    median_perpos_perread = raw_data.groupby(['contig', 'position','reference_kmer', 'read_name']).agg({'event_level_mean':['mean', 'count']})
    median_perpos_perread.columns = ['median', 'coverage']
